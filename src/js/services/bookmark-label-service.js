...
// References: users/{uid}/workspaces/{wid}/bookmarkLabels/{lid}

export const bookmarkLabelService = {
    getCollectionRef(uid, wid) {
        return collection(db, 'users', uid, 'workspaces', wid, 'bookmarkLabels');
    },
    ...
    async update(uid, wid, lid, updates) {
        const labelRef = doc(db, 'users', uid, 'workspaces', wid, 'bookmarkLabels', lid);
