...
// References: users/{uid}/workspaces/{wid}/labels/{lid}

export const labelService = {
    getCollectionRef(uid, wid) {
        return collection(db, 'users', uid, 'workspaces', wid, 'labels');
    },
    ...
    async update(uid, wid, lid, updates) {
        const labelRef = doc(db, 'users', uid, 'workspaces', wid, 'labels', lid);
    ...
    async updateOrders(uid, wid, orderedLabelIds) {
        ...
        orderedLabelIds.forEach((lid, index) => {
            const labelRef = doc(db, 'users', uid, 'workspaces', wid, 'labels', lid);
            // spacing by 100 so there's room to insert later if needed
    ...
    async ensureNoLabelExists(uid, wid) {
        // Check if a system "No Label" bucket already exists
        ...
        // Create the system "No Label" bucket
        const labelRef = doc(this.getCollectionRef(uid, wid));
        const data = {
            id: labelRef.id,
            name: 'No Label',
            color: '#9ca3af',
            order: Number.MAX_SAFE_INTEGER, // Always at the end
            isSystem: true,
            createdAt: serverTimestamp()
        };
        await setDoc(labelRef, data);
