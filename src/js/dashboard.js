...
export class Dashboard {
    ...
    getTableTasks() {
        ...
        return tasks.filter(task => {
            // Parked tasks stay out of the normal table, but a matching parked
            // task must be discoverable when the user searches for it.
            if (task.parked === true && !this.searchQuery) return false;
    ...
    renderTableView() {
        ...
        groupList.forEach(group => {
            ...
            tableRows.forEach(({ task, secondaryGroup, secondaryCollapseKey, secondaryCollapsed }) => {
                ...
                const isCompletedSearchResult = this.searchIncludeCompleted
                    && this.searchCompletedTasks.some(completedTask => completedTask.id === task.id);
                const bucket = this.getTaskBucket(task);
                const tags = this.getTaskTags(task);
    ...
    render() {
        ...
        visibleLabels.forEach((label) => {
            ...
            bucketTasks.forEach(task => {
                ...
                const isParked = task.parked === true;
                const inactiveLabel = task.completed ? 'Completed' : (task.archived ? 'Archived' : '');
                cardsHtml += `
                    <div class="task-card ${isStarred ? 'task-starred' : ''} ${isInactive ? 'task-inactive' : ''} ${isParked ? 'task-parked' : ''}" data-task-id="${task.id}" draggable="${!isInactive}">
                        <div class="task-card-title-row">
                            ...
                            <div class="task-card-actions">
                                ...
                                ${isInactive ? '' : `<button class="btn-icon btn-park-card ${isParked ? 'parked-active' : ''}" data-task-id="${task.id}" data-tooltip="${isParked ? 'Remove from Tray' : 'Park Task'}" style="padding: 0;">
        ...
        parkBtns.forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                try {
                    await taskService.update(this.uid, this.workspaceId, this.boardId, taskId, {
                        parked: newParked,
                        parkedOrder: newParked ? Date.now() : 0
                    });
                } catch (err) {
    ...
    renderParkedTasksTray() {
        ...
        parkedTasks.forEach(task => {
            ...
            // Click chip -> open in task detail
            ...
            // X button -> remove from tray (unpark)
